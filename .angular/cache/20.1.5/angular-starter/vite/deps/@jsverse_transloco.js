...
function translateSignal(key, params, lang, injector) {
  ...
  const result = runInInjectionContext(injector, () => {
    ...
    return toObservable(computerKeysAndParams(key, params)).pipe(switchMap((dynamic) => service2.selectTranslate(dynamic.key, dynamic.params, scope)));
  });
...
function translateObjectSignal(key, params, lang, injector) {
  ...
  const result = runInInjectionContext(injector, () => {
    const service2 = inject(TranslocoService);
    const scope = resolveScope(lang);
    return toObservable(computerKeysAndParams(key, params)).pipe(switchMap((dynamic) => service2.selectTranslateObject(dynamic.key, dynamic.params, scope)));
  });
  return toSignal(result, {
    initialValue: Array.isArray(key) ? [] : {}
  });
